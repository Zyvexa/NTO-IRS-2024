...
camera_matrix_usb = np.array([[716.64132282,   0.        , 423.01790483],
       [  0.        , 715.04014304, 319.1218535 ],
       [  0.        ,   0.        ,   1.        ]])
dist_coefs_usb = np. array([ 0.01556648, -0.0267615 ,  0.00175413,  0.00307516,  0.01450295])
...
while True:
    ...
    try:
        ...
        if M1['m00'] != 0 and M2['m00'] != 0:
            ...
            cropped_image_copy = frame[cy-30:cy+30, cx-30:cx+30]
            global_pl_y = cy - 30
            gl_pl_x = cx - 30
    except: pass
    ...
    try:
        ...
        try:
            ...
            cv2.drawContours(cropped_image_copy, good[0], -1, (255,0,0), 2)
            cv2.drawContours(cropped_image_copy, good[1], -1, (0,255,0), 2)
            cv2.drawContours(cropped_image_copy, good[2], -1, (0,0,255), 2)
            cv2.drawContours(cropped_image_copy, good[3], -1, (255,255,255), 2)
            ...
            if M1['m00'] != 0 and M2['m00'] != 0 and M3['m00'] != 0 and M4['m00'] != 0:
                cx1 = int(M1['m10'] / M1['m00'])
                cy1 = int(M1['m01'] / M1['m00'])
                cv2.circle(cropped_image_copy, (cx1,cy1), 1, (255,0,0), 1)

                cx2 = int(M2['m10'] / M2['m00'])
                cy2 = int(M2['m01'] / M2['m00'])
                cv2.circle(cropped_image_copy, (cx2,cy2), 1, (0,255,0), 1)

                cx3 = int(M3['m10'] / M3['m00'])
                cy3 = int(M3['m01'] / M3['m00'])
                cv2.circle(cropped_image_copy, (cx3,cy3), 1, (0,0,255), 1)

                cx4 = int(M4['m10'] / M4['m00'])
                cy4 = int(M4['m01'] / M4['m00'])
                cv2.circle(cropped_image_copy, (cx4,cy4), 1, (255,255,255), 1)
